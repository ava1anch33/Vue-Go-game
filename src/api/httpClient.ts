...
class HttpClient {
  ...
  private async request<T>(method: HttpMethod, url: string, options: HttpOptions = {}): Promise<T> {
...
export const FetchPost = <T>(url: string, body?: any, options?: HttpOptions) =>
  http.post<T>(url, body, options)
export const FetchPut = <T>(url: string, body?: any, options?: HttpOptions) =>
